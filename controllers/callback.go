...
func (c *CallbackController) UserTransactionCallback() {
	...
	// 	return
	// }
	if resp, err := models.GetUserTransactionsById(transactionRef); err == nil {
		logs.Info("Request ID: ", resp.TransactionId, " found for callback processing")
		if resp != nil {
			// Update the transaction status
			statusCode := v.Status
			...
			if err := models.UpdateUserTransactionsById(resp); err != nil {
				logs.Info("Failed to update transaction status: %v", err)
				responseCode = 505
				responseMessage = "Failed to update transaction status"
				resp := responses.CallbackResponse{
				...
				querySearch := "UserTransactionId:" + resp.TransactionId
				// query: k:v,k:v
				if v := querySearch; v != "" {
					for _, cond := range strings.Split(v, ",") {
						kv := strings.SplitN(cond, ":", 2)
				...
				var insResponses []responses.UserInsTransactions

				insTxns, err := models.GetAllUserInsTransactions(query, fields, sortby, order, offset, limit)
				if err == nil {
					for _, v := range insTxns {
						logs.Info("Payment history found: ", v)
						insTxnObj := v.(models.UserInsTransactions)

						insTxnObj.Status = status
						insTxnObj.DateModified = time.Now()

						if err := models.UpdateUserInsTransactionsById(&insTxnObj); err != nil {
							logs.Error("Failed to update payment history: %v", err)
						} else {
							logs.Info("Payment history updated successfully for Payment ID: ", resp.TransactionId)
						}

						txnResp := responses.UserInsTransactions{
							UserInsTransactionId:   insTxnObj.UserInsTransactionId,
							UserTransactionId:      insTxnObj.UserTransactionId,
							Amount:                 insTxnObj.Amount,
							Data:                   insTxnObj.Data,
							SenderAccountNumber:    insTxnObj.SenderAccountNumber,
							RecipientAccountNumber: insTxnObj.RecipientAccountNumber,
						...
						insResponses = append(insResponses, txnResp)
					}
	...
	resp := responses.UserTxnCallbackResponse{
		StatusCode:    responseCode,
		StatusMessage: responseMessage,
		Result:        &transaction,
	}
